...
'ceres env check --help'`)
	...
	for _, e := range bins {
		...
		if e.exists {
			log.Success("[ceres-env]: %q is installed", e.name)
			continue
		}
		...
		if install {
			install := func() {
				...
				if err != nil {
					...
					log.Success("[ceres-env]: %q is already installed in %q", e.name, path)
				}
			}
			if force {
				install()
