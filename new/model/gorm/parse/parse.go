...
// Parse 创建sql解析器
func Parse(files []string) (*Statement, error) {
	var tables []*Table
	for _, s := range files {
		...
		for {
			...
			if ok {
				tables = append(tables, ParseTable(createTable))
			}
		}
	}
	return &Statement{
...
func ParseTable(table *sqlparser.CreateTable) *Table {
	//
	res := &Table{
		Table:   table.Table.Name.String(),
		Options: []string{},
		Fields:  []*Field{},
	}
	...
	for _, column := range table.TableSpec.Columns {
		field := new(Field)
		field.Name = utils.CamelString(column.Name.String())
		field.Number = false
